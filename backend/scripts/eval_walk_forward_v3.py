...
CONFIDENCE_THRESHOLDS = {
    'BTCUSDT': 0.55,  # V3 adaptive binary - lowered for more trades
    'ETHUSDT': 0.50,  
    'BNBUSDT': 0.45   
}
...
def evaluate_window_v2(symbol, df, window_start, window_end, ensemble, features,
    ...
    for i in range(len(test_df) - latency_bars - 200):
        ...
        # V3 Binary: Skip if confidence is in neutral zone (0.45-0.55)
        if 0.45 <= confidence <= 0.55:
            continue
        
        # Check confidence threshold (for LONG: >0.55, for SHORT: <0.45)
        if confidence >= 0.55:
            # LONG signal
            ...
            if (1 - confidence) < confidence_threshold:
                continue
            direction = -1
...
def evaluate_symbol_v2(symbol, data_dir, models_dir, latency_bars=1, slippage_bps=3):
    ...
    for idx, row in splits_df.iterrows():
        ...
        # V3 splits don't have _idx columns, need to convert dates to indices
        train_start_date = pd.to_datetime(row['train_start']).tz_localize('UTC')
        train_end_date = pd.to_datetime(row['train_end']).tz_localize('UTC')
        test_start_date = pd.to_datetime(row['test_start']).tz_localize('UTC')
        test_end_date = pd.to_datetime(row['test_end']).tz_localize('UTC')
        
        test_start = int(df[df['timestamp'] >= test_start_date].index.min())
        test_end = int(df[df['timestamp'] <= test_end_date].index.max())
        
        # Load model
        model_path = models_dir / symbol / f"ensemble_v3_window_{window_id}.pkl"
...
def main():
    ...
    models_dir = Path(__file__).parent.parent.parent / "models" / "ensemble_v3"
