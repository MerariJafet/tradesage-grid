...
Usage:
    python train_ensemble_v3.py --symbols BTCUSDT ETHUSDT BNBUSDT --trials 30
"""
import argparse
import sys
import json
...
def get_feature_columns(df):
    ...
    exclude = [
        ...
        # V3 adaptive binary specific columns
        'label', 'vol_regime', 'forward_return', 'adaptive_threshold', 
        'threshold_value', 'atr'  # Exclude 'atr' (lowercase), keep 'ATR_14' as feature
    ]
...
def train_symbol_v2(symbol, data_dir, output_dir, n_trials=30):
    ...
    # Load dataset
    dataset_files = list(data_dir.glob(f"{symbol}_v3_*.parquet"))
    if not dataset_files:
        print(f"   ERROR: No V3 dataset found for {symbol}")
        return None
    
    # Use adaptive version
    dataset_file = [f for f in dataset_files if 'adaptive' in f.name]
    if not dataset_file:
    ...
    # Load splits
    splits_path = data_dir / f"{symbol}_v3_splits.csv"
    ...
    # V3 splits have dates, convert to indices
    df['timestamp'] = pd.to_datetime(df['timestamp'])
    
    for idx in splits_df.index:
        train_start_date = pd.to_datetime(splits_df.loc[idx, 'train_start']).tz_localize('UTC')
        train_end_date = pd.to_datetime(splits_df.loc[idx, 'train_end']).tz_localize('UTC')
        test_start_date = pd.to_datetime(splits_df.loc[idx, 'test_start']).tz_localize('UTC')
        test_end_date = pd.to_datetime(splits_df.loc[idx, 'test_end']).tz_localize('UTC')
        
        splits_df.loc[idx, 'train_start_idx'] = int(df[df['timestamp'] >= train_start_date].index.min())
        splits_df.loc[idx, 'train_end_idx'] = int(df[df['timestamp'] <= train_end_date].index.max())
        splits_df.loc[idx, 'test_start_idx'] = int(df[df['timestamp'] >= test_start_date].index.min())
        splits_df.loc[idx, 'test_end_idx'] = int(df[df['timestamp'] <= test_end_date].index.max())
    
    splits_df['window'] = splits_df['window_id']
    ...
    for idx, row in splits_df.iterrows():
        ...
        train_start = int(row['train_start_idx'])
        train_end = int(row['train_end_idx'])
        test_start = int(row['test_start_idx'])
        
        # Split train into train/val (80/20)
        val_size = int((train_end - train_start) * 0.2)
        val_start = int(train_end - val_size)
        
        X_train = df.iloc[train_start:val_start][features].values
        y_train = df.iloc[train_start:val_start]['label'].values
        
        X_val = df.iloc[val_start:train_end][features].values
        y_val = df.iloc[val_start:train_end]['label'].values
    ...
    for model_info in models:
        model_path = symbol_dir / f"ensemble_v3_window_{model_info['window']}.pkl"
        joblib.dump({
            'base_models': {
...
def main():
    ...
    # Directories
    data_dir = Path(__file__).parent.parent.parent / "data" / "datasets_v3"
    output_dir = Path(__file__).parent.parent.parent / "models" / "ensemble_v3"
